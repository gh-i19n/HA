...
import { useIsMobile } from "@healthalst/ui/hooks";
import {
  Root as TooltipRoot,
  TooltipProvider,
  TooltipTrigger,
...
const SIDEBAR_WIDTH = "calc(var(--spacing) * 80)";
const SIDEBAR_WIDTH_MOBILE = "calc(var(--spacing) * 68)";
const SIDEBAR_WIDTH_ICON = "calc(var(--spacing) * 14)";
    ...
    return (
      <div
        ...
        className={cn(
          ...
          "flex h-1/2 w-(--sidebar-width) flex-col",
          className,
        )}
        {...properties}
      >
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...properties}>
        <SheetContent
          ...
          data-mobile="true"
          className={cn(
            "bg-sidebar border-none bg-sidebar-bg text-sidebar-foreground w-(--sidebar-width) p-0 [&>button]:hidden",
            className,
          )}
          style={
            {
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ? "overflow-hidden p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(5)))]"
            : "group-data-[collapsible=icon]:w-(--sidebar-width-icon)",
          className,
        )}
        {...properties}
      >
        <div
          ...
          className={cn(
            "flex h-full w-full flex-col bg-sidebar bg-sidebar-bg",
...
/**
 * The icon button that expands/collapses the sidebar. Wraps MainButton so it
 * gets the shared hover/focus/disabled treatment for free, and calls
 * `toggleSidebar` from context after any caller-supplied `onClick` runs.
 */
function SidebarTrigger({
  ...
  return (
    ...
    >
      {isMobile ? (
        openMobile ? (
          <X size={17} className={cn("stroke-2")} />
        ) : (
          <Menu size={17} className={cn("stroke-2")} />
        )
      ) : open ? (
        <ChevronLeft size={16} className={cn("stroke-3")} />
      ) : (
        <ChevronRight size={16} className={cn("stroke-3")} />
      )}
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "text-sidebar-foreground outline-hidden ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
...
/** Renders a stable navigation placeholder without layout-shifting random widths. */
