...
- Real laboratory organizations and branch/tenant boundaries. Product vocabulary
  uses **Laboratory** for the tenant concept previously labelled clinic/imaging
  centre; the generic `organizations` persistence model is unchanged.
- Laboratory self-registration (laboratory name, location, address, owner
  credentials). Laboratory-created staff accounts with role assignment
  (`OWNER`, `ADMIN`, `REPORT_STAFF`) and app-issued credentials.
- Public patient booking journey: a patient without an account picks a
  registered laboratory, requests a test/appointment, and the platform
  provisions a single patient account (one per email across all laboratories)
  with an app-generated password.
- Laboratory appointment approval with emailed show-up feedback; payment and
  clinical procedure management remain outside this product slice.
- Reports become available to the patient by the platform emailing a portal link
  plus the one-time password (included only before the patient's first sign-in),
  so each patient has exactly one account across all laboratories.
...
- Changing, paying for, or cancelling bookings once approved; and clinical
  procedure management. Patients may request and laboratories may approve
  appointments; nothing in this slice bills, reschedules, or cancels them.
- Diagnostic decision support, treatment advice, automated clinical claims, or
  allowing an unqualified user to clinically finalize a report. The product may
  provide structured document templates and rendering; an authorized imaging
  professional remains responsible for all clinical content and finalization.
- Diagnostic-image viewing or DICOM object storage.
- General patient messaging, report sharing, referring-clinician access,
  marketing, and population analytics. Transactional in-product notifications
  about account, membership, upload, publication, correction, and withdrawal
  events are in scope.
- Emergency or support impersonation. A user may belong to multiple clinics,
  but every request has one explicit active clinic and permissions are evaluated
  from that clinic membership only.
...
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Laboratory report administrator | Within assigned laboratory/branch: find eligible bookings; list reports; upload, publish, correct, supersede, and withdraw reports; inspect lifecycle audit needed for the job | Any other laboratory; patient account administration unless separately authorized; changing report authorship or clinical content outside the approved workflow; deleting history |
| Patient                     | View/download available reports linked to that patient's verified account and bookings; see own appointment status; exercise supported privacy/account rights          | Pending, quarantined, rejected, withdrawn, superseded, other-patient, or other-laboratory data; administrator operations                                                          |
| Laboratory tenant administrator | Provision/deprovision staff and assign least-privilege laboratory roles through the approved identity model; create staff accounts with app-issued credentials          | Report content access merely because the actor administers accounts; cross-laboratory access                                                                                      |
...
Laboratory memberships use the tailored Eventorch role model: `OWNER` owns the
laboratory workspace, `ADMIN` manages members and laboratory settings,
`REPORT_STAFF` works with bookings and reports, and `PATIENT` is an account type
rather than a laboratory staff role. A user may have multiple memberships but
cannot combine permissions across active laboratory contexts.
...
### FR-002 — Provision and revoke laboratory staff access

- **Source:** DISC real clinic/operating boundary; least privilege.
- **Actor/outcome:** A laboratory tenant administrator creates named staff
  accounts and assigns roles with fixed permission sets; staff credentials are
  issued by the platform, not chosen by the laboratory or the staff member.
- **Preconditions/trigger:** The laboratory and tenant administrator are
  verified; staff employment/authority is established outside or through an
  approved onboarding process.
- **Required behavior:** Provisioning creates the staff account (display name,
  email, role in `ADMIN` or `REPORT_STAFF`), generates a strong password
  platform-side, emails the credentials once, and records actor, tenant,
  subject, role, reason, timestamp, and effective state. Role change,
  suspension, and revocation follow the same audited path. Role changes must
  affect new authorization decisions without requiring data repair. Staff
  self-registration is not part of the product; accounts exist only when a
  laboratory creates them.
- **Failure/recovery:** Duplicate or stale changes are idempotent or conflict;
  the last valid administrator for a laboratory cannot be removed without an
  approved recovery path; a failed credential email leaves the account usable
  through an approved reset path that never reveals the generated password in
  logs.
...
- **Acceptance criteria:** Tests prove cross-laboratory denial, least-privilege
  role mapping, session revocation after suspension, immutable audit,
  one-time credential delivery, and safe recovery from a locked-out tenant.
...
### FR-003 — Provision one patient account per email across laboratories

- **Source:** DISC identity and booking-matching risk; OUT-002.
- **Actor/outcome:** A patient gains exactly one platform account (keyed by
  verified email) regardless of how many laboratories they book; the account is
  provisioned by the platform at booking time, never self-registered.
- **Preconditions/trigger:** A patient requests an appointment through the
  public booking journey, or a laboratory needs an account for an existing
  patient of a booking.
- **Required behavior:** The platform finds the existing account for the email
  or creates one, generates a strong password, stores only the hash, and
  emails the portal link plus password as part of the report-availability
  notification (included only while the patient has never signed in). A
  booking's `patient_user_id` always references this single account, so reports
  from every laboratory land in the same patient inbox.
- **Failure/recovery:** Duplicate booking for the same email links to the same
  account; case/whitespace normalization prevents duplicate accounts; a failed
  credential email never blocks report publication and follows an approved
  recovery path; the generated password is never logged.
- **Authorization/data:** Patient self-assertion, names, phone/email possession,
  or booking reference alone cannot establish ownership of another person's
  account or reports; booking-linked access is granted only through the single
  verified account.
- **Acceptance criteria:** Tests cover first booking (account created), second
  laboratory booking (account reused), duplicate email normalization, credential
  email once, no plaintext password in logs, and cross-laboratory report
  aggregation on the same account.
- **Verification:** Domain/integration tests, threat-model scenarios, and user
  journey validation.
- **Priority/dependencies/owner/status:** P0 / DEC-002, booking contract /
  product, security, backend, QA / draft.
...
- **Required behavior:** The server must re-evaluate session, patient link,
  ...
  inline preview or attachment disposition, filename, caching, and browser
  security headers apply. Patients and authorized clinic staff can preview a
  report before download. Canonical structured reports are downloadable as PDF
  and DOCX; original uploads remain available in their validated source format.
  Every successful/denied access is audited without logging content.
...
### FR-016 — Register a laboratory

- **Source:** Product-owner scope correction on 2026-08-14.
- **Actor/outcome:** A laboratory registers itself and enters its workspace;
  patients never self-register and staff never self-register.
- **Required behavior:** The registration form is laboratory-only with no role
  tab switcher. It collects the owner's name, email, and password plus the
  laboratory name, location, and address, and atomically creates the
  organization with an `OWNER` membership and a signed-in session. Patients are
  provisioned by the platform during the public booking journey (FR-003);
  staff accounts are created by a laboratory administrator (FR-002). Seeded
  users and newly registered users use the same repositories and session model.
- **Failure/recovery:** Duplicate email, weak password, partial laboratory
  creation, and retry are safe and produce field-level recovery.
- **Acceptance criteria:** Integration and browser tests prove laboratory
  signup, absence of patient/staff self-registration surfaces, duplicate
  denial, rollback, login, logout, and persistence across restart.
- **Priority/dependencies/owner/status:** P0 / FR-001–FR-003 / backend,
  frontend, security, QA / draft.

### FR-017 — Select and administer a clinic tenant

- **Source:** Product-owner scope expansion; Eventorch organization/membership
  implementation.
- **Actor/outcome:** Multi-clinic users select one active clinic; owners/admins
  manage clinic profile, invitations, members, roles, suspension, and removal.
- **Required behavior:** The active tenant is explicit in the authenticated
  context. All staff report queries and mutations derive tenant identity from
  that context, never from an untrusted client ownership field. Membership
  changes are auditable and immediately affect authorization; the last active
  owner cannot be removed or demoted.
- **Acceptance criteria:** Tests cover switching, role matrices, stale sessions,
  cross-tenant IDs, invitation lifecycle, last-owner protection, and member
  suspension.
- **Priority/dependencies/owner/status:** P0 / FR-001, FR-002, FR-016 /
  backend, frontend, security, QA / draft.

### FR-018 — Manage account, clinic, security, and notification settings

- **Source:** Product-owner scope expansion; Eventorch settings implementation.
- **Actor/outcome:** Users manage profile and notification preferences while
  authorized clinic admins manage clinic and member settings.
- **Required behavior:** Reusable settings navigation exposes profile, security
  and active sessions, notification preferences, clinic profile, and team/roles
  according to account type and active-tenant permission. Server authorization
  remains authoritative and sensitive changes are audited.
- **Acceptance criteria:** Component/API tests cover success, validation,
  unauthorized tabs/actions, concurrent changes, and persisted preferences.
- **Priority/dependencies/owner/status:** P0 / FR-001, FR-017 / frontend,
  backend, QA / draft.

### FR-019 — Deliver durable in-product notifications and live updates

- **Source:** Product-owner scope expansion; Eventorch notification/SSE
  implementation.
- **Actor/outcome:** Patients and clinic members receive timely, role-appropriate
  updates after relevant report, membership, and account actions.
- **Required behavior:** A committed domain action creates a durable user
  notification and emits an authenticated SSE signal. Notification payloads
  contain safe display data and opaque resource identifiers, not report content.
  The client reconnects, resumes from the last event where supported, refreshes
  affected queries, shows unread state, and lets the user mark notifications
  read. Heartbeats and bounded cleanup prevent stale emitters.
- **Acceptance criteria:** Tests prove post-commit delivery, reconnect,
  duplicate tolerance, unread/read behavior, tenant and patient isolation,
  notification preferences, and no PHI leakage in event logs or URLs.
- **Priority/dependencies/owner/status:** P0 / FR-007, FR-010–FR-011,
  FR-017–FR-018 / backend, frontend, security, QA / draft.

### FR-020 — Create reports from reusable clinical document templates

- **Source:** Product-owner scope expansion on 2026-08-14.
- **Actor/outcome:** Authorized report staff can create a consistently
  structured clinically final document associated with an eligible booking.
- **Required behavior:** The first release includes reusable Chest X-ray and MRI
  Brain templates with clinic identity, patient/booking context, examination,
  clinical indication, technique, comparison, findings, impression, reporting
  professional, and finalization metadata. Structured content is validated and
  versioned, then rendered server-side to PDF and DOCX from one canonical model.
  Template UI and rendering do not make diagnostic suggestions.
- **Acceptance criteria:** Tests verify required fields, authorization,
  booking/tenant binding, deterministic PDF/DOCX rendering, safe filenames,
  preview, publication, correction, and seeded representative examples.
- **Priority/dependencies/owner/status:** P0 / FR-004, FR-007, FR-009 /
  domain, backend, frontend, QA / draft.

### FR-021 — Public laboratory directory and patient booking request

- **Source:** Product-owner journey correction on 2026-08-14.
- **Actor/outcome:** A patient without an account chooses a registered
  laboratory from a public list and requests an appointment for a checkup or
  test; the request is created and the patient is emailed a booking
  acknowledgement.
- **Preconditions/trigger:** The laboratory completed self-registration; the
  patient supplies name, email, laboratory, test/examination type, and
  preferred date.
- **Required behavior:** `GET /public/laboratories` returns only registered
  laboratories with name, location, and address. `POST /public/appointments`
  validates inputs, provisions the single patient account (FR-003), creates a
  booking in `REQUESTED` state owned by the selected laboratory, and sends a
  confirmation email with what happens next. Requests never require an account,
  and an invalid/unknown laboratory is denied without disclosing other tenants.
- **Failure/recovery:** Duplicate submission for the same email reuses the
  account and creates a distinct booking; invalid email/laboratory/date produce
  bounded validation; booking spam is an acknowledged residual risk with
  rate-limiting deferred.
- **Authorization/data:** The public surface never returns patient data, staff
  data, booking identifiers, or report metadata; creation is the only mutation.
- **Acceptance criteria:** Tests cover directory listing, first and repeat
  booking, unknown laboratory, invalid input, acknowledgement email, and no
  unauthorized data on the public surface.
- **Priority/dependencies/owner/status:** P0 / FR-003, FR-004 / backend,
  frontend, security, QA / draft.

### FR-022 — Laboratory appointment approval with emailed feedback

- **Source:** Product-owner journey correction on 2026-08-14.
- **Actor/outcome:** Laboratory staff see pending appointment requests, approve
  or reject them, and the patient receives the show-up time by email.
- **Preconditions/trigger:** A booking is in `REQUESTED` state and belongs to
  the active laboratory; authorized staff review it.
- **Required behavior:** Staff list bookings with status and patient contact;
  approval sets `APPROVED` plus a scheduled time and emails the patient the
  show-up feedback; rejection sets `REJECTED`, emails a reason, and never
  creates a report-eligible record. Approving or rejecting a booking that is no
  longer `REQUESTED` conflicts and is rejected.
- **Failure/recovery:** Concurrent approval/rejection resolves to one outcome;
  email failure never leaves the booking in an unrecorded state; staff cannot
  act on another laboratory's bookings.
- **Authorization/data:** Only staff with the laboratory's report permission may
  act; status transitions are server-enforced.
- **Acceptance criteria:** Tests cover list, approve, reject, conflict, stale
  state, cross-laboratory denial, and both email paths.
- **Priority/dependencies/owner/status:** P0 / FR-021 / backend, frontend,
  QA / draft.

### FR-023 — App-issued patient credentials with report availability

- **Source:** Product-owner journey correction on 2026-08-14.
- **Actor/outcome:** When a laboratory publishes a result, the platform emails
  the patient a portal link and the account password (only before first
  sign-in), so the patient signs into one account for all laboratories.
- **Preconditions/trigger:** A report transitions to `AVAILABLE` for a booking
  owned by the patient.
- **Required behavior:** Publication sends the report-available email extended
  with the portal link and the generated password when the patient has never
  signed in (`last_login_at IS NULL`); later publications send only the link.
  The password is generated at provisioning (FR-003), stored only as a hash,
  and never logged or sent by the laboratory.
- **Failure/recovery:** A failed credential email does not block publication;
  the patient can still sign in with the app-issued password and an approved
  reset path covers lost credentials; no plaintext password reaches logs,
  metrics, or traces.
- **Authorization/data:** The laboratory never sees or transmits patient
  credentials; only the platform holds the provisioning path.
- **Acceptance criteria:** Tests prove the credential email is sent once, the
  second report only links, cross-laboratory reports appear on the same
  account, and no plaintext credential is observable in application output.
- **Priority/dependencies/owner/status:** P0 / FR-003, FR-007, FR-021 /
  backend, security, QA / draft.
...
| FR-016–FR-018                                                                          | PO 2026-08-14, Eventorch reference implementation             | Account type, organization membership, active-tenant and settings contracts    | Registration, invitation, role matrix, tenant-isolation and settings journeys    |
| FR-019                                                                                 | PO 2026-08-14, Eventorch notification/SSE implementation      | Durable notification model and authenticated SSE contract                      | Commit-order, reconnect, deduplication, preference and isolation tests            |
| FR-009, FR-020                                                                         | PO 2026-08-14                                                  | Canonical structured report model, secure preview and render/export contract   | Template validation plus PDF/DOCX rendering, header, authorization and browser tests |
...
| DEC-004 | Supported report formats and limits                                                | First release accepts clinically final PDF uploads up to 20 MiB and canonical structured Chest X-ray/MRI Brain reports. Structured reports render to PDF and DOCX; files use signature/MIME validation, private storage, secure preview/download, and malware controls where applicable. DICOM remains deferred. | Approve scanner/storage/preview/render architecture and verify malicious-file and output-escaping controls.                                            |
| DEC-005 | Clinical finalization, publication, correction, withdrawal, and patient visibility | Reports may be finalized upstream and attached, or authored from a neutral structured template by an authorized clinic professional. Templates provide structure only and no diagnostic suggestion. Corrections create immutable versions; patients see only the current available version; withdrawal removes access while preserving records/audit. | Obtain clinical/domain approval and trace it to lifecycle, template, concurrency, audit, revocation, and patient-content tests.                       |
