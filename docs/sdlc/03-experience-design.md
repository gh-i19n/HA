...
No representative staff or patient usability session or assistive-technology
user session exists yet. The product owner accepted the implemented Eventorch
interaction system as the adaptation baseline for this interview project; real
user validation remains required verification evidence and is not implied by
passing this design gate.

## Cycle 3 approved Eventorch adaptation

The product owner selected Eventorch's implemented dashboard experience as the
authoritative visual and interaction reference. HealthAlst reuses its responsive
dashboard shell, floating/collapsible sidebar, sticky topbar, breadcrumbs,
notification bell, avatar account menu, settings shell, forms, dialogs, data
tables, empty/error/loading states, and document viewer through shared,
prop-driven components. Domain labels, navigation, permissions, and data come
from HealthAlst contracts; clinic names, users, roles, menu items, metrics, and
report fields are never embedded inside shell primitives.

The primary information architecture is:

- public: sign in, laboratory registration, public appointment booking
  (no account required), and privacy-safe recovery;
- laboratory workspace: overview, appointments (approve/reject with show-up
  feedback), reports, report templates, notifications, and settings;
- patient workspace: results, appointments, notifications, profile, security,
  and notification settings;
- laboratory settings: laboratory profile, members and roles/permissions,
  account profile, security/sessions, and notification preferences.

Registration is laboratory-only: a single form (owner name, email, password,
laboratory name, location, address) with no role tab switcher. Patients never
self-register; the platform provisions one patient account per email during the
public booking journey and emails the portal link plus the one-time password
when the first report becomes available. Staff accounts are created by a
laboratory administrator with role assignment; the platform emails their
credentials.

The topbar always exposes the active clinic switcher for multi-membership staff,
the notification bell, and the reusable avatar menu with profile, settings,
theme, and sign-out actions. The sidebar is the Eventorch sidebar composition
tailored to the routes above, with permission-filtered navigation, desktop
collapse, mobile sheet behavior, and no hash-based pseudo-navigation.

Report access uses an Eventorch-derived document viewer: explicit preview opens
an authorized inline PDF in a focus-managed dialog/page; download is a separate
action with PDF and DOCX choices when the report has canonical structured
content. Staff authoring uses a reusable template form shell and field registry,
with Chest X-ray and MRI Brain schemas; preview and validation occur before
publication. The UI never supplies diagnostic suggestions.

Notifications are durable inbox items. SSE is only a refetch signal, so a lost
connection cannot lose the underlying notification. Bell count/list, read-one,
read-all, reconnect, empty, offline, and permission states follow Eventorch's
patterns while using tenant-safe HealthAlst query keys.
...
2. **Orientation:** The default `Imaging results` page explains that it contains
   reports made available by issuing laboratories. It does not promise that
   all expected or pending reports are shown.
3. **List:** Available reports are ordered newest service date first, with
   laboratory, imaging service, service date, report date, and a single
   `View report` action. Reports can be filtered by the laboratory that issued
   them. Report files are not fetched with the list.
4. **Open:** The patient selects a report. If step-up authentication is required,
   ...
   copy.
5. **Review context:** The report page identifies the issuing laboratory, imaging
...
7. **Support:** Adjacent content says: `Questions about what this report means?
 Contact the laboratory that issued it.` The support route never asks the
   patient to email the report or credentials.
8. **Return:** Back navigation returns to the preserved patient list position.

#### Journey F — Book an appointment with a registered laboratory

1. **Entry:** Anyone (no account required) opens the public booking page.
2. **Choose laboratory:** A dropdown lists registered laboratories with name,
   location, and address. Only self-registered laboratories appear.
3. **Describe the request:** The patient selects/enters the test or checkup type
   and a preferred date, then provides their name and email.
4. **Submit:** The platform validates the input, creates the booking request in
   `REQUESTED` state, and provisions the patient account (one per email) without
   asking for a password.
5. **Confirmation:** The page confirms the request was received and explains
   that the laboratory will email the show-up time. A confirmation email states
   the same and never contains a password.
6. **Approval feedback:** When laboratory staff approve, an email gives the
   scheduled show-up date/time plus any message; a rejection email explains why.
7. **Result access:** When the laboratory publishes the result, the platform
   emails the portal link and — for first-time sign-in — the one-time password.
   The patient signs in to one account and sees results from every laboratory
   they used.
...
- If identity linking is missing or ambiguous, report access remains blocked.
  ...
  laboratories, bookings, or reports.
...
- The pending appointment request layout and interaction rationale are recorded
  in the [Pending appointment request card design note](../product/pending-appointment-request-card.md),
  traced to FR-022.
